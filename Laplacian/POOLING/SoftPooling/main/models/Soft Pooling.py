...
class SoftPool2d(nn.Module):
    ...
    def forward(self, x):
        ...
        unfolded = nn.functional.unfold(x, kernel_size=self.kernel_size, stride=self.stride)
        unfolded = unfolded.view(batch_size, channels, self.kernel_size * self.kernel_size, -1)
        weights = self.softmax(unfolded)
        pooled = (unfolded * weights).sum(dim=2)
        pooled = pooled.view(batch_size, channels, out_height, out_width)
        return pooled
...
class ResNet(nn.Module):
    def __init__(self, num_classes):
        ...
        self.softpool = SoftPool2d(kernel_size=2, stride=2)
